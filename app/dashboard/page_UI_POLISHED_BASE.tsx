      ...
      >
        ...
        <div className="px-3 py-3">
          <button
            ...
            className="group flex w-full items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-[#4965ff] via-[#6652e8] to-[#a044e8] px-4 py-3 text-sm font-semibold text-white shadow-[0_8px_30px_rgba(92,78,220,.22)] transition-all duration-200 hover:-translate-y-0.5 hover:shadow-[0_12px_38px_rgba(92,78,220,.35)] active:translate-y-0 active:scale-[0.99]"
          >
          ...
          <div className="ml-auto flex items-center gap-1.5 sm:gap-2">
            ...
            <div ref={helpMenuRef} className="relative">
              <button
                ...
                title="Help"
                aria-label="Help"
                className={`rounded-xl p-2.5 ${isDark ? 'text-white/55 hover:bg-white/[0.05]' : 'text-black/50 hover:bg-black/[0.04]'}`}
              >
                <span className="flex h-4 w-4 items-center justify-center rounded-full border border-current text-[10px] font-bold">?</span>
              </button>
              {showHelpMenu && (
                <div className={`absolute right-0 top-11 z-50 w-64 rounded-2xl border p-3 shadow-2xl ${isDark ? 'border-white/[0.10] bg-[#121722]' : 'border-black/[0.08] bg-white'}`}>
                  <div className="px-2 py-1">
                    <p className="text-xs font-semibold">AIOS Help</p>
                    <p className={`mt-1 text-[11px] leading-5 ${isDark ? 'text-white/45' : 'text-black/50'}`}>
                      Ask anything, attach files, switch models, or enable Web Search for up-to-date answers.
                    </p>
                  </div>
                </div>
              )}
                    ...
                    <div className="mt-7 grid w-full max-w-xl grid-cols-1 gap-2 sm:grid-cols-2">
                      {SUGGESTIONS.map((suggestion, i) => (
                        <button key={i} onClick={() => handleSuggestionClick(suggestion.prompt)} className={`flex items-center gap-3 rounded-2xl border p-4 text-left transition hover:-translate-y-0.5 ${isDark ? 'border-white/[0.08] bg-white/[0.025] hover:border-violet-400/25 hover:bg-white/[0.045]' : 'border-black/[0.08] bg-white hover:border-violet-300 hover:bg-violet-50/50'}`}>
                          <span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl text-lg transition-transform duration-200 group-hover:scale-105 ${isDark ? 'bg-violet-500/10' : 'bg-violet-50'}`}>{suggestion.icon}</span>
                        ...
                        {editingMessageIndex === i && msg.role === 'user' ? (
                          ...
                          <div className={isAI ? 'w-full' : 'flex justify-end'}>
                            <div className={isAI ? 'w-full' : 'max-w-[88%]'}>
                              {isAI ? (
                                <div className={`rounded-[22px] border p-4 sm:p-5 transition-shadow duration-300 ${isDark ? 'border-white/[0.10] bg-gradient-to-br from-[#151a22] to-[#11161e] shadow-[0_18px_50px_rgba(0,0,0,.18)]' : 'border-black/[0.07] bg-white shadow-[0_14px_38px_rgba(15,23,42,.07)]'}`}>
                                  ...
                                  {msg.web_search && msg.sources?.length > 0 && (
                                    <div className={`mt-4 border-t pt-3 ${isDark ? 'border-white/[0.08]' : 'border-black/[0.07]'}`}>
                                      <div className={`mb-2 text-[10px] font-semibold uppercase tracking-wide ${isDark ? 'text-white/40' : 'text-black/40'}`}>Sources</div>
                                      <div className="flex flex-wrap gap-2">
                                        {msg.sources.slice(0, 6).map((source: any, sourceIndex: number) => (
                                          <a
                                            key={sourceIndex}
                                            href={source.uri}
                                            target="_blank"
                                            rel="noopener noreferrer"
                                            className={`max-w-full truncate rounded-lg border px-2.5 py-1.5 text-[10px] transition ${isDark ? 'border-white/[0.08] text-white/55 hover:bg-white/[0.05] hover:text-white' : 'border-black/[0.08] text-black/55 hover:bg-black/[0.03] hover:text-black'}`}
              ...
              <div className="mx-auto w-full max-w-[900px]">
                ...
                <div className="relative mb-2.5 flex items-center">
                  <button
                    ...
                    className={`group flex items-center gap-2 rounded-full border px-3 py-1.5 text-xs font-medium shadow-sm transition-all duration-200 active:scale-[0.97] hover:-translate-y-[1px] ${
                      showModelPicker
                ...
                <div className={`relative rounded-[26px] border p-2 shadow-[0_14px_48px_rgba(0,0,0,.11)] transition-all duration-200 focus-within:-translate-y-[1px] focus-within:border-violet-500/45 focus-within:ring-4 focus-within:ring-violet-500/[0.07] ${isDark ? 'border-white/[0.10] bg-[#11151d] shadow-[0_14px_48px_rgba(0,0,0,.24)]' : 'border-black/[0.08] bg-white'}`}>
                  <textarea
                    ref={messageInputRef}
                    id="message-input"
                    value={prompt}
                    ...
                    rows={1}
                    className={`min-h-[58px] max-h-[220px] w-full resize-none overflow-y-auto bg-transparent px-2 pb-14 pt-2 text-[15px] leading-6 outline-none transition-colors ${isDark ? 'text-white placeholder:text-white/35' : 'text-black placeholder:text-black/35'}`}
                  />

                  {isListening && <div className="absolute left-4 top-3.5 flex items-center gap-1.5 rounded-full border border-red-400/20 bg-red-500/10 px-2 py-1 text-[10px] font-semibold text-red-400 shadow-sm"><span className="h-2 w-2 animate-pulse rounded-full bg-red-500" /> REC</div>}
                  ...
                  <div className="absolute bottom-2 left-2 right-2 flex items-center justify-between">
                    <div className="flex items-center gap-0.5 sm:gap-1">
                      ...
                      <button
                        ...
                        className={`hidden items-center gap-1.5 rounded-xl px-2.5 py-2 text-xs transition lg:flex ${isWebSearchEnabled ? 'bg-violet-500/10 text-violet-400' : isDark ? 'text-white/60 hover:bg-white/[0.05]' : 'text-black/60 hover:bg-black/[0.04]'}`}
                      >
                        <Globe className="h-4 w-4" />
                        Web Search
                        {isWebSearchEnabled && <span className="h-1.5 w-1.5 rounded-full bg-violet-400" />}
                      </button>
                    </div>

                    <div className="flex items-center gap-1">
                      <button onClick={toggleVoiceInput} title={isListening ? 'Stop voice input' : 'Voice input'} className={`rounded-xl p-2.5 transition-all duration-200 active:scale-95 ${isListening ? 'bg-red-500 text-white shadow-[0_0_22px_rgba(239,68,68,.30)]' : isDark ? 'text-white/60 hover:bg-white/[0.06] hover:text-white' : 'text-black/55 hover:bg-black/[0.04] hover:text-black'}`}>
                      ...
                      {(isLoading || isTyping) ? (
                        ...
                        <button onClick={() => handleSubmit()} disabled={!prompt.trim() && uploadedFiles.length === 0} title="Send" className={`rounded-xl p-2.5 text-white transition-all duration-200 active:scale-95 ${prompt.trim() || uploadedFiles.length > 0 ? 'bg-gradient-to-r from-[#4d62ff] to-[#7a4de8] shadow-lg shadow-violet-500/20 hover:-translate-y-0.5 hover:scale-[1.03] hover:shadow-violet-500/30' : isDark ? 'bg-white/[0.07] text-white/30' : 'bg-black/[0.06] text-black/25'}`}><Send className="h-4 w-4" /></button>
                      )}
                    </div>
                  </div>
                </div>
